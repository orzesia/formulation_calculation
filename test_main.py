...
class DilutionCalculatorTests(unittest.TestCase):
    ...
    def test_mix_includes_shared_vial_dilution_and_shared_diluent(self):
        mix = calculate_mix(
            [
                {"virus": "PEDV", "target_ct": 30.0},
                {"virus": "PDCoV", "target_ct": 30.5},
            ],
            final_volume_ul=3000.0,
        )
        ...
        self.assertEqual(mix["components"][0]["additional_dilution_factor"], 27)
        self.assertEqual(mix["components"][0]["total_dilution_factor"], 108)
        self.assertAlmostEqual(mix["components"][0]["rna_volume_ul"], 111.11, places=2)
        self.assertAlmostEqual(mix["shared_diluent_volume_ul"], 1388.89, places=2)

    def test_shared_vial_uses_full_final_volume_for_stock_volume(self):
        result = calculate_component(
            stock_ct=27.95,
            target_ct=30.5,
            final_volume_ul=3000.0,
        ...
        self.assertEqual(result["total_dilution_factor"], 8)
        self.assertEqual(result["additional_dilution_factor"], 2)
        self.assertEqual(result["shared_vial_dilution_factor"], 2)
        self.assertAlmostEqual(result["rna_volume_ul"], 1500.0, places=2)
